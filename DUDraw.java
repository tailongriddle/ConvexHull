/******************************************************************************
 *  Compilation:  javac DUDraw.java
 *  Execution:    java DUDraw
 *  Dependencies: none
 *
 * Forked from  <a href = "https://introcs.cs.princeton.edu/java/stdlib/javadoc/StdDraw.html">Princeton's StdDraw</a>,
 * maintained for Denver University's COMP 1000 program.
 *
 *
...
/**
 *
 *  DUDraw is based heavily on StdDraw. (https://introcs.cs.princeton.edu/java/stdlib/StdDraw.java.html) by Robert Sedgewick and Kevin Wayne, copyright 2019
 *  
 *  The {@code DUDraw} class provides a basic capability for
 *  creating drawings with your programs. It uses a simple graphics model that
 ...
 *  <p>
 *  <b>Getting started.</b>
 *  To use this class, you must have {@code DUDraw.class} in your
 *  Java classpath. Follow the instructions on the <a href = "https://git.cs.du.edu/super-serious-software/dudraw/-/wikis/start-java">Git Wiki Page</a>
 *  to get started.
 *  Now, type the following short program into your editor:
 *  <pre>
 *   public class TestDUDraw {
 *       public static void main(String[] args) {
 *           DUDraw.setPenRadius(0.5);
 *           DUDraw.setPenColor(DUDraw.BLUE);
 *           DUDraw.point(0.5, 0.5);
 *           DUDraw.setPenColor(DUDraw.MAGENTA);
 *           DUDraw.line(0.2, 0.2, 0.8, 0.2);
 *       }
 *   }
 *  </pre>
 ...
 *  The methods {@code DUDraw.line()} and {@code DUDraw.point()}
 *  draw lines and points; the methods {@code DUDraw.setPenRadius()}
 *  and {@code DUDraw.setPenColor()} control the line thickness and color.
 *  <p>
 ...
 *  and represented in degrees. For example, {@code DUDraw.arc(0.0, 0.0, 1.0, 0, 90)}
 ...
 *  is 0.5 and is not affected by coordinate scaling. This default pen
 *  radius is half the width of a pixel, so that if
 *  you draw x amount of points equally spaced along a horizontal or vertical line
 *  where x = the width or height of the canvas divided by 2,
 *  you will be able to see individual circles, but if you draw twice that many
 *  points, the result will look like a line.
 *  <ul>
 *  <li> {@link #setPenRadius(double radius)}
 *  </ul>
 *  <p>
 *  For example, {@code DUDraw.setPenRadius(5)} makes
 *  *  the thickness of the lines and the size of the points to be ten times
 *  *  the 0.5 default.
 *  *  To draw points with the minimum possible radius (one pixel on typical
 *  *  displays), set the pen radius to the default value of 0.5.
 *  <p>
 *  <b>Pen color.</b>
 ...
 *  For example, {@code DUDraw.setPenColor(DUDraw.MAGENTA)} sets the
 ...
 *  For example, {@code DUDraw.setCanvasSize(800, 800)}
 ...
 *  leave a small margin, you can call {@code DUDraw.setScale(-.05, 1.05)}.
 *  <p>
 ...
 *   DUDraw.setFont(font);
 *   DUDraw.text(0.5, 0.5, "Hello, World");
 *  </pre>
 *  <p>
 *  <b>Images.</b>
 ...
 *  {@code DUDraw.clear(DUDraw.LIGHT_GRAY)} clears the canvas to a shade
 *  of gray.
 *  <p>
 ...
 *   @author Eddy Rogers
 */
public final class DUDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
...
//    private static final double DEFAULT_PEN_RADIUS = 0.002;
    private static final double DEFAULT_PEN_RADIUS = 0.5;
    ...
    private static DUDraw std = new DUDraw();
    ...
    private static double mouseXDragTracking = 0;
    private static double mouseYDragTracking = 0;
    private static int mousePressedHistory = 0;
    private static int mouseReleasedHistory = 0;
     ...
     *
     * @return the current value of the width of the canvas
     */
     ...
     *
     * @return the current value of the height of the canvas
     */
    public static int getCanvasHeight() { return height; }

    /**
     * Returns an array containing the min and max of the x scale
     * @return [xmin, xmax]
     */
    ...
    /**
     * Returns an array containing the min and max of the y scale
     * @return [ymin, ymax]
     */
    ...
    /**
     * Closes the DUDraw Window.
     */
    public static void closeWindow() {
        //If we need to call event listeners, look into the following line:
    ...
    ***************************************************************************/
    ...
    /**
     * Clears the screen to the given RGB color.
     *
     * @param red the amount of red for the new screen color (Between 0 and 255)
     * @param blue the amount of blue for the new screen color (Between 0 and 255)
     * @param green green, blue the color values to make the background
     * @throws IllegalArgumentException if {@code color} is {@code null}
     */
    ...
    /**
     * Sets the pen size to the default size (0.5).
     * The pen is circular, so that lines have rounded ends, and when you set the
     * pen radius and draw a point, you get a circle of the specified radius.
     * This value is the radius, not the diameter, so a radius of 0.5 will
     * produce a line with a thickness of 1 pixel, while a radius of 1 will
     * produce a line with a thickness of 2 pixels.
    ...
    public static void setPenRadius(double radius) {
        ...
        //Normalize pen radius so that 1 = 1 pixel
        radius /= 256;
     ...
     * {@code DUDraw.BLACK}, {@code DUDraw.BLUE}, {@code DUDraw.CYAN},
     * {@code DUDraw.DARK_GRAY}, {@code DUDraw.GRAY}, {@code DUDraw.GREEN},
     * {@code DUDraw.LIGHT_GRAY}, {@code DUDraw.MAGENTA}, {@code DUDraw.ORANGE},
     * {@code DUDraw.PINK}, {@code DUDraw.RED}, {@code DUDraw.WHITE}, and
     * {@code DUDraw.YELLOW}.
     *
    ...
    ***************************************************************************/
    ...
    /**
     * Returns the color of a pixel at a given x and y value
     *
     * @param  pixelX the x-value of the pixel to be queried
     * @param  pixelY the y-value of the pixel to be queried
     * @return the color of the pixel at the x and y values specified
     */
    public static Color getPixelColor(double pixelX, double pixelY) {

        // Convert to screen coordinates, and multiply by 2 for the raster
        double finalY = scaleY(pixelY) * 2;

        // Convert to screen coordinates, and multiply by 2 for the raster
        double finalX = scaleX(pixelX) * 2;

        //Query the pixel from the raster
        int pixelData = onscreenImage.getRGB((int)finalX, (int)finalY);
        //Get the ColorModel to decode the result from the raster
        ColorModel colorModel = onscreenImage.getColorModel();

        //Return a new color with the decoded values from the raster
        return new Color(colorModel.getRed(pixelData), colorModel.getGreen(pixelData), colorModel.getBlue(pixelData), colorModel.getAlpha(pixelData));
    }
     ...
     *
     * @param  x0 the <em>x</em>-coordinate of the first vertex
     * @param  y0 the <em>y</em>-coordinate of the first vertex
     * @param  x1 the <em>x</em>-coordinate of the second vertex
     * @param  y1 the <em>y</em>-coordinate of the second vertex
     * @param  x2 the <em>x</em>-coordinate of the third vertex
     * @param  y2 the <em>y</em>-coordinate of the third vertex
     * @throws IllegalArgumentException if any coordinate is either NaN or infinite
     */
    ...
    /**
     * Draws a filled triangle with vertices at 
     * (<em>x</em><sub>0</sub>, <em>y</em><sub>0</sub>),
     * (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>), and
     * (<em>x</em><sub>2</sub>, <em>y</em><sub>2</sub>).
     *
     ...
     * @throws IllegalArgumentException if any coordinate is either NaN or infinite
     */
    public static void filledTriangle(double x0, double y0, double x1, double y1, double x2, double y2) {
    	validate(x0, "x0");
    	validate(x1, "x1");
    	validate(x2, "x2");
    	validate(y0, "y0");
    ...
    /**
     * Draws a quadrilateral with vertices at 
     * (<em>x</em><sub>0</sub>, <em>y</em><sub>0</sub>),
     * (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>),
     * (<em>x</em><sub>2</sub>, <em>y</em><sub>2</sub>), and
     ...
     * @param  x2 the <em>x</em>-coordinate of the third vertex
     * @param  y2 the <em>y</em>-coordinate of the third vertex
     * @param  x3 the <em>x</em>-coordinate of the fourth vertex
     * @param  y3 the <em>y</em>-coordinate of the fourth vertex
     * @throws IllegalArgumentException if any coordinate is either NaN or infinite
     */
    public static void quadrilateral(double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3) {
    	validate(x0, "x0");
    	validate(x1, "x1");
    	validate(x2, "x2");
    	validate(x3, "x3");
    ...
    /**
     * Draws a filled quadrilateral with vertices at 
     * (<em>x</em><sub>0</sub>, <em>y</em><sub>0</sub>),
     * (<em>x</em><sub>1</sub>, <em>y</em><sub>1</sub>),
     * (<em>x</em><sub>2</sub>, <em>y</em><sub>2</sub>), and
     * (<em>x</em><sub>3</sub>, <em>y</em><sub>3</sub>).
     ...
     * @throws IllegalArgumentException if any coordinate is either NaN or infinite
     */
    public static void filledQuadrilateral(double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3) {
    	validate(x0, "x0");
    	validate(x1, "x1");
    	validate(x2, "x2");
    	validate(x3, "x3");
    ...
    /**
     * Draws an elliptical arc of the specified semimajor and semiminor axes,
     * centered at (<em>x</em>, <em>y</em>), from angle1 to angle2 (in degrees).
     *
     * @param  x the <em>x</em>-coordinate of the center of the ellipse
     * @param  y the <em>y</em>-coordinate of the center of the ellipse
     ...
     * @param  angle2 the angle at the end of the arc. For example, if
     *         you want a 90 degree arc, then angle2 should be angle1 + 90.
     * @throws IllegalArgumentException if {@code semiMajorAxis} or {@code semiMinorAxis} is negative
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void ellipticalArc(double x, double y, double semiMajorAxis, double semiMinorAxis, double angle1, double angle2) {
        validate(x, "x");
        validate(y, "y");
        validate(semiMajorAxis, "semimajor axis");
        validate(semiMinorAxis, "semiminor axis");
    ...
    /**
     * Draws a circular sector of the specified radius,
     * centered at (<em>x</em>, <em>y</em>), from angle1 to angle2 (in degrees).
     *
     * @param  x the <em>x</em>-coordinate of the center of the circle
     * @param  y the <em>y</em>-coordinate of the center of the circle
     ...
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void sector(double x, double y, double radius, double angle1, double angle2) {
        validate(x, "x");
        validate(y, "y");
        validate(radius, "arc radius");
        validate(angle1, "angle1");
        ...
        if (ws <= 1 && hs <= 1) pixel(x, y);
        	...
        	Arc2D.Double arc = new Arc2D.Double(xs - ws/2, ys - hs/2, ws, hs, angle1, angle2 - angle1, Arc2D.OPEN);
        	GeneralPath path = new GeneralPath();
        	path.append(arc.getPathIterator(new AffineTransform()), true);
        	path.lineTo(xs, ys);
        	path.closePath();
        	offscreen.draw(path);
        }
    ...
    /**
     * Draws a filled circular sector of the specified radius,
     * centered at (<em>x</em>, <em>y</em>), from angle1 to angle2 (in degrees).
     *
     * @param  x the <em>x</em>-coordinate of the center of the circle
     * @param  y the <em>y</em>-coordinate of the center of the circle
     ...
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void filledSector(double x, double y, double radius, double angle1, double angle2) {
        validate(x, "x");
        validate(y, "y");
        validate(radius, "arc radius");
        validate(angle1, "angle1");
    ...
    /**
     * Draws an elliptical sector of the specified semimajor and semiminor axes,
     * centered at (<em>x</em>, <em>y</em>), from angle1 to angle2 (in degrees).
     *
     * @param  x the <em>x</em>-coordinate of the center of the circle
     * @param  y the <em>y</em>-coordinate of the center of the circle
     ...
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void ellipticalSector(double x, double y, double semiMajorAxis, double semiMinorAxis, double angle1, double angle2) {
        validate(x, "x");
        validate(y, "y");
        validate(semiMajorAxis, "semimajor axis");
        validate(semiMinorAxis, "semiminor axis");
    ...
    /**
     * Draws a filled elliptical sector of the specified semimajor and semiminor axes,
     * centered at (<em>x</em>, <em>y</em>), from angle1 to angle2 (in degrees).
     *
     * @param  x the <em>x</em>-coordinate of the center of the circle
     * @param  y the <em>y</em>-coordinate of the center of the circle
     ...
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void filledEllipticalSector(double x, double y, double semiMajorAxis, double semiMinorAxis, double angle1, double angle2) {
        validate(x, "x");
        validate(y, "y");
        validate(semiMajorAxis, "semimajor axis");
        validate(semiMinorAxis, "semiminor axis");
    ...
    /**
     * Draws an annulus of the specified radiuses, centered at (<em>x</em>, <em>y</em>).
     *
     * @param  x the <em>x</em>-coordinate of the center of the annulus
     * @param  y the <em>y</em>-coordinate of the center of the annulus
     * @param  radius1 the radius of one of the circles of the annulus
     * @param  radius2 the radius of the other circle of the annulus
     * @throws IllegalArgumentException if {@code radius1} or {@code radius2} is negative
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void annulus(double x, double y, double radius1, double radius2) {
        validate(x, "x");
        validate(y, "y");
        validate(radius1, "radius1");
        validateNonnegative(radius1, "radius1");
        validate(radius2, "radius2");
    ...
    /**
     * Draws a filled annulus of the specified radiuses, centered at (<em>x</em>, <em>y</em>).
     *
     * @param  x the <em>x</em>-coordinate of the center of the annulus
     * @param  y the <em>y</em>-coordinate of the center of the annulus
     * @param  radius1 the radius of one of the circles of the annulus
     ...
     * @throws IllegalArgumentException if any argument is either NaN or infinite
     */
    public static void filledAnnulus(double x, double y, double radius1, double radius2) {
        validate(x, "x");
        validate(y, "y");
        validate(radius1, "radius1");
        validateNonnegative(radius1, "radius1");
        ...
        double xs = scaleX(x);
        double ys = scaleY(y);
        double ws1 = factorX(2*radius1);
        double hs1 = factorY(2*radius1);
        double ws2 = factorX(2*radius2);
        double hs2 = factorY(2*radius2);

        if (ws1 <= 1 && hs1 <= 1) pixel(x, y);
        else {
            Ellipse2D.Double circle1 = new Ellipse2D.Double(xs - ws1/2, ys - hs1/2, ws1, hs1);
            Ellipse2D.Double circle2 = new Ellipse2D.Double(xs - ws2/2, ys - hs2/2, ws2, hs2);
            GeneralPath path = new GeneralPath();
            path.append(circle1.getPathIterator(new AffineTransform()), true);
            path.append(circle2.getPathIterator(new AffineTransform(new double[] {1, 0, 0, -1, 0, 2 * ys})), true);
            offscreen.fill(path);
        }
    ...
    ***************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        // in case file is inside a .jar (classpath relative to DUDraw)
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = DUDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
...
*/
    ...
    ***************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(DUDraw.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            DUDraw.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    ***************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = DUDraw.userX(e.getX());
            mouseY = DUDraw.userY(e.getY());
            isMousePressed = true;
            if(mousePressedHistory < Integer.MAX_VALUE) mousePressedHistory++;

            mouseXDragTracking = mouseX();
            mouseYDragTracking = mouseY();
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {
        synchronized (mouseLock) {
            ...
            if(mouseReleasedHistory < Integer.MAX_VALUE) mouseReleasedHistory++;
        }
    }
    ...
    /**
     * Returns the number of mouse press events that have occurred since the last time getMousePressed() was called.
     * @return number of mouse press events
     */
    public static int getMousePressed()
    {
        //Stop if there is no mouse event history
        int temp = 0;
        if(mousePressedHistory > 0) { temp = mousePressedHistory; }
        mousePressedHistory = 0;
        return temp;
    }

    /**
     * Returns the number of mouse release events that have occurred since the last time getMouseReleased() was called.
     * @return number of mouse release events
     */
    public static int getMouseReleased()
    {
        //Stop if there is no mouse event history
        int temp = 0;
        if(mouseReleasedHistory > 0) { temp = mouseReleasedHistory; }
        mouseReleasedHistory = 0;
        return temp;
    }

    /**
     * Returns whether the mouse position has been changed, and if the mouse is still pressed since the last time getMouseDragged() was called.
     * @return whether or not the mouse has been dragged
     */
    public static boolean getMouseDragged()
    {
        //If the mouse is not pressed, the mouse cannot be dragged
        if(!isMousePressed) { return false; }

        //If either mouse value has changed since last time, it is a drag
        if (mouseXDragTracking != mouseX || mouseYDragTracking != mouseY)
        {
            mouseXDragTracking = mouseX();
            mouseYDragTracking = mouseY();
            return true;
    ...
    ***************************************************************************/
    ...
    public static void main(String[] args) {
        ...
        setCanvasSize(2048, 1024);
        DUDraw.setPenColor(DUDraw.BLACK);
        DUDraw.filledRectangle(0.25, 0.25, 0.25, 0.25);

        DUDraw.setPenColor(DUDraw.YELLOW);
        DUDraw.filledRectangle(0.75, 0.25, 0.25, 0.25);

        DUDraw.setPenColor(DUDraw.GREEN);
        DUDraw.filledRectangle(0.25, 0.75, 0.25, 0.25);

        DUDraw.setPenColor(DUDraw.BLUE);
        DUDraw.filledRectangle(0.75, 0.75, 0.25, 0.25);

        DUDraw.setPenColor(DUDraw.WHITE);
        DUDraw.setPenRadius(3);
        DUDraw.line(0.25, 0.5, 0.25, 0.5);
        DUDraw.line(0.5, 0.5, 0.5, 0.5);
        DUDraw.line(0.75, 0.5, 0.75, 0.5);

        System.out.println(getPixelColor(0.25, 0.25));
        System.out.println(getPixelColor(0.75, 0.25));
        System.out.println(getPixelColor(0.25, 0.75));
        System.out.println(getPixelColor(0.75, 0.75));
